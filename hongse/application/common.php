...
function get_ip_area($ip)
{
    ...
    return substr11($info);  //ArrayHelper是助手函数 可以将下面的方法追加到上面
}
 ...
 * @return  [json]               [json数据]
 */
function DataReturn($msg = '', $code = 0, $data = '',$token = null) {
    // 默认情况下，手动调用当前方法
    $result = ['msg' => $msg, 'code' => $code, 'data' => $data,$token];
 ...
 * @return
 */
function CreateToken($user_token)
{
    $key = "hu";  //这里是自定义的一个随机字串，应该写在config文件中的，解密时也会用，相当    于加密中常用的 盐  salt
    $token = [
        "iss" => "hyy",  //签发者 可以为空
        "aud" => "wkx", //面象的用户，可以为空
        "iat" => time(), //签发时间
        ...
        "user_token" => $user_token //记录的userid的信息，这里是自已添加上去的，如果有其它信息，可以再添加数组的键值对
    ];
...
function checkToken($token)
{
    try{
        ...
        if(!empty($Result['user_token'])){
            return DataReturn('验证成功',2000,$Result);
        }
    }catch(Exception $e){

        return DataReturn('验证失败',-1000);
    }
}
 ...
 * @return
 */
function CreateAdminToken($phone)
{
 ...
 * 省份数据
 */
function ProvinceDataList()
{
    $List  = Db::name('region')->where(['pid'=>100000])->select();
    return $List;
}
 ...
 * 省市区所有数据
 */
function LocationAllData()
{
    $data = [];
    $field = 'id,name,pid';
    $data['Province']  = Db::name('region')->where('pid','<>',0)->where('level',1)->field($field)->select();
    foreach ($data['Province'] as $key=> &$item){
        $item['City'] = Db::name('region')->where(['pid'=>$item['id']])->field($field)->select();
        foreach ($item['City'] as $k=> &$value){
            $value['area'] = Db::name('region')->where(['pid'=>$value['id']])->field($field)->select();
        }
    }
    return $data;
}
 ...
 * 城市数据
 */
function CityDataList($province_id = null)
{
    $where = [];
 ...
 * 随意查询
 */
function OptionalQuery($table,$where=[])
{
 ...
 * 随意查询数量
 */
function OptionalQueryCount($table,$where=[])
{
 ...
 * 记录用户浏览记录
 */
function recordHistory($userToken,$module,$product_id)
{
    $record = [
    ...
    $check = Db::name('brow_history')->where(['user_token'=>$userToken,'module'=>$module,'product_id'=>$product_id,])->find();
    if(!empty($check)){
        Db::name('brow_history')->where(['user_token'=>$userToken,'module'=>$module,'product_id'=>$product_id,])->update(['add_time'=>time()]);
    }else{
        Db::name('brow_history')->insert($record);
    }
...
{
    $List = \app\models\StadiumModel::where(['status'=>1])->field('id,building_name')->select();
    return $List;
}
...
{
    $place = Db::name('region')->where(['id'=>$place_id])->value('name');
    return $place;
}
...
function banner($id)
{
    $Img = Db::name('web_images')->where(['id'=>$id])->value('images');
    if(!empty($Img)){
        $Img = http_type().$Img;
    }
    return $Img;
