...
//            }
            ...
            Cache::set('backup_config', $config,0);
            //生成备份文件信息
            $file = [
                'name' => date('Ymd-His', $Request->time()),
                'part' => 1
            ];
            Cache::set('backup_file', $file,0);
            //缓存要备份的表
            Cache::set('backup_tables', $ids,0);
            //创建备份文件
            ...
            //备份指定表
            $Database = new \com\Database(Cache::get('backup_file'), Cache::get('backup_config'));
            ...
            if (false === $start) { //出错
                ...
                if (isset($tables[++$id])) {
                    ...
                    unlink(Cache::get('backup_config')['path'] . 'backup.lock');
                    $tab = implode ('|',Cache::get('backup_tables'));
                    $time = Cache::get('backup_file')['name'];
                    writelog('备份数据库【'.$time.'】成功',200);
                    Cache::rm('backup_tables');
                    Cache::rm('backup_file');
                    Cache::rm('backup_config');
    ...
    public function optimize($ids = null) {
        ...
        if (count($table) > 1) {
            ...
            if($list){
                writelog('用户【'.$this->Administrator['username'].'】优化数据库成功',200);
//                $this->success("数据表优化完成！");
                return json(['code'=>200,'msg'=>"数据表优化完成！"]);
            } else {
                writelog('用户【'.$this->Administrator['username'].'】优化数据库失败',100);
...
//                $this->success("数据表'{$ids}'优化完成！");
                writelog('用户【'.$this->Administrator['username'].'】优化【'.$ids.'】表成功',200);
                return json(['code'=>200,'msg'=>"数据表'{$ids}'优化完成！"]);
                //return json("数据表'{$ids}'优化完成！");
            } else {
//                $this->error("数据表'{$ids}'优化出错请重试！");
                writelog('用户【'.$this->Administrator['username'].'】优化【'.$ids.'】表失败',100);
    ...
    public function repair($ids = null) {
        ...
        if (count($table) > 1) {
            ...
            if($list){
                writelog('用户【'.$this->Administrator['username'].'】修复数据库成功',200);
//                $this->success("数据表修复完成！");
                return json(['code'=>200,'msg'=>"数据表修复完成！"]);
            } else {
                writelog('用户【'.$this->Administrator['username'].'】修复数据库失败',100);
...
//                $this->success("数据表'{$ids}'修复完成！");
                writelog('用户【'.$this->Administrator['username'].'】修复【'.$ids.'】表成功',200);
                return json(['code'=>200,'msg'=>"数据表'{$ids}'修复完成！"]);
            } else {
//                $this->error("数据表'{$ids}'修复出错请重试！");
                writelog('用户【'.$this->Administrator['username'].'】修复【'.$ids.'】表失败',100);
    ...
    public function revert($time = 0, $part = null, $start = null) {
        if (!empty($time) && is_null($part) && is_null($start)) { //初始化
            ...
            if (count($list) === $last[0]) {
                Cache::set('backup_list', $list); //缓存备份列表
