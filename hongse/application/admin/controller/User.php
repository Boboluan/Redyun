...
class User extends Base
{
    ...
    public function userEdit()
    {
        ...
        if(request()->isPost()){
            ...
            if($res['code'] == 200){
                $param['portrait'] = $res['msg'];
                //判断编辑的是不是自己的头像
                if($this->Administrator['uid']==$param['id']){
                    $have = "have";
                }
            }elseif($res['code'] == 100){
    ...
    public function adminEdit(){
        ...
        if(input('type')=="checkPassword"){
            $flag =  $user->checkOldPassword ($oldpassword,$this->Administrator['uid']);
            return json(['code' => $flag['code'], 'data' => $flag['data'], 'msg' => $flag['msg']]);
        }else{
            $param = input('post.');
            if(empty($param['password'])){
    ...
    public function editPwd(){
        ...
        if(isset($type) && $type=='checkPassword'){
            ...
            $flag =  $user->checkOldPassword ($old_pwd,$this->Administrator['uid']);
            return json(['code'=>$flag['code'],'msg'=>$flag['msg']]);
        }else{
    ...
    public function batchDelUser(){
        ...
        if(in_array($this->Administrator['uid'],$ids)){
            $key = array_search ($this->Administrator['uid'],$ids);
            unset($ids[$key]);
            if(empty($ids)){
