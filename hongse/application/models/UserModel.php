...
class UserModel extends Model
{
    ...
    public function editUser($param)
    {
        ...
        try{
            ...
            $status = '';
            if($param['id']==$this->Administrator['uid']){
                Cache::set('portrait', $param['portrait']); //用户头像
    ...
    public function editPassword($param){
        $name = $this->where('id',$this->Administrator['uid'])->value('username');
        Db::startTrans();// 启动事务
        try{
            $this->allowField (true)->save($param,['id'=>$this->Administrator['uid']]);
            Db::commit();// 提交事务
    ...
    public function forbiddenAdmin($param){
        ...
        try{
            if($param){
                ...
                $this->where('id','not in',[1,$this->Administrator['uid']])->update(['status'=>2]);
            }
            Db::commit();// 提交事务
